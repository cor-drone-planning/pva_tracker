...
void pvaCheckerCallback(const trajectory_msgs::JointTrajectoryPoint::ConstPtr& msg)
{
...
int main(int argc, char** argv) {
    ...
    ros::Subscriber pos_cmd_sub = nh.subscribe("/position_cmd", 1, posCmdCallback);
    ros::Subscriber pva_checker = nh.subscribe("/pva_setpoint", 1, pvaCheckerCallback);
    ...
    ros::ServiceClient set_mode_client = nh.serviceClient<mavros_msgs::SetMode>
            ("mavros/set_mode");
    ros::ServiceClient set_home = nh.serviceClient<mavros_msgs::CommandHome>("mavros/cmd/set_home");
    ...
    while(ros::ok()){
        ...
        mavros_msgs::CompanionProcessStatus cp_msg;
        cp_msg.header.stamp = ros::Time::now();
        cp_msg.component = 196;  // MAV_COMPONENT_ID_AVOIDANCE
        cp_msg.state = 4;  // MAV_STATE::MAV_STATE_ACTIVE
        system_status_pub.publish(cp_msg);

        // trajectory_msgs::JointTrajectoryPoint set_position;
        trajectory_msgs::JointTrajectoryPointPtr ptr_set_position(new trajectory_msgs::JointTrajectoryPoint());
        // ptr_set_position = &set_position;
        ptr_set_position->positions.push_back(take_off_position.pose.position.x);
        ptr_set_position->positions.push_back(take_off_position.pose.position.y);
        ptr_set_position->positions.push_back(take_off_position.pose.position.z);
        ptr_set_position->velocities.push_back(0);
        ptr_set_position->velocities.push_back(0);
        ptr_set_position->velocities.push_back(0);
        ptr_set_position->accelerations.push_back(0);
        ptr_set_position->accelerations.push_back(0);
        ptr_set_position->accelerations.push_back(0);

        pvaCallback(ptr_set_position);
