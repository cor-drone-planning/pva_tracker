/**
 * @file tracker_sim_auto_arm_takeoff.cpp
 * @author Clearance Chen, Siyuan Wu (siyuanwu99@gmail.com)
 * @brief
 * @version 1.0
 * @date 2022-02-26
 *
...
void pvaCheckerCallback(
    const trajectory_msgs::JointTrajectoryPoint::ConstPtr &msg) {
  pva_topic_received = true;
}

void posCmdCallback(const quadrotor_msgs::PositionCommand &msg) {
  pva_topic_received = true;

  mavros_msgs::AttitudeTarget att_setpoint;
  planned_p << msg.position.x, msg.position.y, msg.position.z;
  planned_v << msg.velocity.x, msg.velocity.y, msg.velocity.z;
  planned_a << msg.acceleration.x, msg.acceleration.y, msg.acceleration.z;
  planned_yaw = msg.yaw;
  ...
  planned_v += vectorElementMultiply(p_error, position_error_p) +
               vectorElementMultiply(delt_p_error, position_error_d) +
               vectorElementMultiply(p_error_accumulate, position_error_i);
  ...
  // Set a maximum acceleration feedforward value given by position and velocity
  // error.
  for (int i = 0; i < 3; i++) {
  ...
  double thrust_des =
      a_des.norm() *
      thrust_factor;  // a_des.dot(att_current_vector) * THRUST_FACTOR
  ...
  ROS_INFO_THROTTLE(
      1.0, "Attitude Quaternion Setpoint is w=%f, x=%f, y=%f, z=%f, thrust=%f",
      att_setpoint.orientation.w, att_setpoint.orientation.x,
      att_setpoint.orientation.y, att_setpoint.orientation.z,
      att_setpoint.thrust);
...
void pvaCallback(const trajectory_msgs::JointTrajectoryPoint::ConstPtr &msg) {
  ...
  // planned_a << -msg->accelerations[1], msg->accelerations[0],
  // msg->accelerations[2];
  ...
  planned_a << msg->accelerations[0], msg->accelerations[1],
      msg->accelerations[2];
  ...
  //    Quaterniond att_current_vector_quat = current_att * z_w_quat *
  //    current_att.inverse(); Vector3d
  //    current_att_vector(att_current_vector_quat.x(),
  //    att_current_vector_quat.y(), att_current_vector_quat.z());

  //    Quaterniond att_des_q = Quaterniond::FromTwoVectors(z_w_norm,
  //    att_des_norm);

  //    //add yaw
  //    Quaterniond yaw_quat(cos(planned_yaw/2.0),
  //    att_des_norm(0)*sin(planned_yaw/2.0),
...
void positionCallback(const geometry_msgs::PoseStamped::ConstPtr &msg) {
  ...
  if (!time_counter_initialized) {
    ...
    last_time = ros::Time::now().toSec();
    // double coeff_stand_by_time_to_flight_time = max_flight_time /
    // max_stand_by_time; thrust_factor = thrust_factor_min +
    // (thrust_factor_max-thrust_factor_min)*(flight_time_second / 60.0 +
    // stand_by_time_second / 60.0*coeff_stand_by_time_to_flight_time) /
    // max_flight_time; if(thrust_factor > thrust_factor_max) thrust_factor =
    // thrust_factor_max;
  }
...
void configureCallback(tracker::PVA_TrackerConfig &config, uint32_t level) {
  position_error_p << config.position_p_xy, config.position_p_xy,
      config.position_p_z;
  position_error_d << config.position_d_xy, config.position_d_xy,
      config.position_d_z;
  position_error_i << config.position_i_xy, config.position_i_xy,
      config.position_i_z;
  p_i_acc_error_limit = config.p_i_acc_error_limit;

  velocity_error_p << config.velocity_p_xy, config.velocity_p_xy,
      config.velocity_p_z;
  velocity_error_d << config.velocity_d_xy, config.velocity_d_xy,
      config.velocity_d_z;
  velocity_error_i << config.velocity_i_xy, config.velocity_i_xy,
      config.velocity_i_z;
  v_i_acc_error_limit = config.v_i_acc_error_limit;
  thrust_factor =
      config.hover_thrust_factor_min;  // initial value. Consider the battery is
                                       // fully charged.
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber position_sub =
      nh.subscribe("/mavros/local_position/pose", 1, positionCallback);
  ros::Subscriber velocity_sub = nh.subscribe(
      "/mavros/local_position/velocity_local", 1, velocityCallback);
  ros::Subscriber pva_sub = nh.subscribe("/pva_setpoint", 1, pvaCallback);
  ros::Subscriber pos_cmd_sub =
      nh.subscribe("/position_cmd", 1, posCmdCallback);
  ros::Subscriber pva_checker =
      nh.subscribe("/pva_setpoint", 1, pvaCheckerCallback);
  att_ctrl_pub = nh.advertise<mavros_msgs::AttitudeTarget>(
      "/mavros/setpoint_raw/attitude", 1);
  odom_sp_enu_pub = nh.advertise<nav_msgs::Odometry>("/odom_sp_enu", 1);
  tracking_error_pub =
      nh.advertise<geometry_msgs::PointStamped>("/pva_tracking_error", 1);

  ros::Timer timer1 = nh.createTimer(ros::Duration(0.2),
                                     watchDogCallback);  // RATE 5 Hz to publish

  ros::Publisher pose_sp_pub = nh.advertise<geometry_msgs::PoseStamped>(
  ...
  current_p << take_off_position.pose.position.x,
      take_off_position.pose.position.y, take_off_position.pose.position.z;
  ...
  ROS_INFO(
      "Position received. Vehicle will get into offboard mode and arm soon.");
  ...
  while (ros::ok()) {
    ...
    if (current_p(2) > 0.6 && pva_topic_received) {
      ROS_WARN("PVA setpoint received, tracking started!");
      break;
    }
  ...
  while (ros::ok()) {
    if (!pva_topic_received) {
      ...
      while ((fabs(current_p(0) - take_off_position.pose.position.x) > 1.0 ||
              fabs(current_p(1) - take_off_position.pose.position.y) > 1.0) &&
             ros::ok()) {
